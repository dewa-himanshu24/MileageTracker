...
const PasscodeScreen = ({ setRedirectToPasscodeScreen, userName, nickname, email }) => {
  ...
  const handleInputChange = (value, index, setState, refsArray, nextRefArray) => {
    setState((prevState) => {
  ...
  const handleSkip = async () => {
    ...
    const upadtedUser = await UserServices.addUser(newUser);
    console.log('Dewa1 PasscodeScreen upadtedUser', JSON.stringify(upadtedUser));
      setUser(upadtedUser);
          ...
          <ScrollView contentContainerStyle={styles.scrollViewContent}>
            <View style={styles.topContainer}>
              <View style={styles.inputContainer}>
                <Text style={[styles.label, { marginBottom: 4 }]}>Enter a 4-Digit Passcode <Text style={styles.required}>*</Text></Text>
                <Text style={[styles.subText, { marginBottom: 12 }]}>You will need to enter it every app launch</Text>
                <View style={styles.passcodeContainer}>
                  {passcode.map((digit, index) => (
                    <TextInput
                      ...
                      value={digit ? 'X' : ''}
                      onChangeText={(value) => handleInputChange(value, index, setPasscode, passcodeRefs, confirmPasscodeRefs)}
                      onKeyPress={(e) => handleKeyPress(e, index, setPasscode, passcodeRefs)}
                      maxLength={1}
                      keyboardType="number-pad"
                      secureTextEntry={false}
                      returnKeyType="next"
                      onSubmitEditing={() => index < 3 && passcodeRefs.current[index + 1].focus()}
                    />
                  ))}
                </View>
                ...
                <View style={styles.passcodeContainer}>
                  {confirmPasscode.map((digit, index) => (
                    <TextInput
                      ...
                      style={[styles.input, styles.inputText]}  
                      value={digit ? 'X' : ''}
                      onChangeText={(value) => handleInputChange(value, index, setConfirmPasscode, confirmPasscodeRefs)}
                      onKeyPress={(e) => handleKeyPress(e, index, setConfirmPasscode, confirmPasscodeRefs)}
                      maxLength={1}
                      keyboardType="number-pad"
                      secureTextEntry={false}
                      returnKeyType="next"
                      onSubmitEditing={() => index < 3 && confirmPasscodeRefs.current[index + 1].focus()}
                    />
                  ))}
                </View>
                {confirmPasscodeError ? <Text style={styles.errorText}>{confirmPasscodeError}</Text> : null}
              </View>
            </View>
          </ScrollView>
        </KeyboardAvoidingView>
